...
app.get('/api/cameras', requireAuth, (req, res) => {
  ...
  if (isAdmin) {
    return res.json(db.prepare(`SELECT ${cols} FROM cameras ORDER BY sort_pos, id`).all());
  }

  const allowed = db.prepare('SELECT camera_id FROM user_cameras WHERE user_id=?')
    .all(req.session.userId).map(r => r.camera_id);
  ...
  return res.json(
    db.prepare(`SELECT ${cols} FROM cameras WHERE id IN (${placeholders}) ORDER BY sort_pos, id`)
      .all(...allowed)
  );
});
...
app.get('/api/users', requireAuth, requireAdmin, (req, res) => {
  const users = db.prepare('SELECT id, username, role, prefs FROM users ORDER BY id').all();
  res.json(users.map(u => {
    let prefs = {};
    try { prefs = JSON.parse(u.prefs || '{}'); } catch {}
    return { id: u.id, username: u.username, role: u.role, lang: prefs.lang || 'ru' };
  }));
});
...
app.post('/api/users', requireAuth, requireAdmin, (req, res) => {
  ...
  const validLang = ['ru', 'en', 'kz', 'by'].includes(lang) ? lang : 'ru';
  const prefs = JSON.stringify({ lang: validLang });
  try {
    const r = db.prepare("INSERT INTO users (username, password, role, prefs) VALUES (?, ?, ?, ?)")
      .run(username.trim(), bcrypt.hashSync(password, 10), role, prefs);
    res.json({ id: r.lastInsertRowid, username: username.trim(), role, lang: validLang });
  } catch (e) {
...
app.put('/api/users/:id/lang', requireAuth, requireAdmin, (req, res) => {
  const id = parseInt(req.params.id);
  const { lang } = req.body || {};
  if (!['ru', 'en', 'kz', 'by'].includes(lang)) return res.status(400).json({ error: 'invalid lang' });
  const user = db.prepare('SELECT prefs FROM users WHERE id=?').get(id);
...
app.put('/api/users/:id/password', requireAuth, requireAdmin, (req, res) => {
  ...
  if (!newPassword || newPassword.length < 4) return res.status(400).json({ error: 'Пароль слишком короткий (мин. 4 символа)' });
...
app.get('/api/users/:id/cameras', requireAuth, requireAdmin, (req, res) => {
  const id = parseInt(req.params.id);
  const allCameras = db.prepare('SELECT id, name, group_name FROM cameras ORDER BY sort_pos, id').all();
  const assigned = db.prepare('SELECT camera_id FROM user_cameras WHERE user_id=?')
    .all(id).map(r => r.camera_id);
  res.json({ cameraIds: assigned, allCameras });
});

app.put('/api/users/:id/cameras', requireAuth, requireAdmin, (req, res) => {
  const id = parseInt(req.params.id);
  const { cameraIds } = req.body || {};
  if (!Array.isArray(cameraIds)) return res.status(400).json({ error: 'cameraIds must be array' });
  db.transaction(() => {
    db.prepare('DELETE FROM user_cameras WHERE user_id=?').run(id);
    const insert = db.prepare('INSERT OR IGNORE INTO user_cameras (user_id, camera_id) VALUES (?, ?)');
    cameraIds.forEach(cid => insert.run(id, parseInt(cid)));
  })();
  res.json({ ok: true });
});
